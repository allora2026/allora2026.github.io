...
class SiteTests(unittest.TestCase):
    ...
    def test_new_posts_cover_requested_topics(self):
        ...
        reviewable_memory_content = REVIEWABLE_MEMORY_NOTE.read_text().lower()
        ...
        start_work_content = START_WORK_NOTE.read_text().lower()
        self.assertIn('start with the work', start_work_content)
        self.assertIn('usable', start_work_content)
        self.assertIn('team memory', start_work_content)
        interface_content = INTERFACE_NOTE.read_text().lower()
        ...
        agent_board_content = AGENT_BOARD_NOTE.read_text().lower()
        self.assertIn('the board is the memory', agent_board_content)
        ...
        checklist_demo_content = CHECKLIST_DEMO_NOTE.read_text().lower()
        self.assertIn('the checklist is the demo', checklist_demo_content)
        self.assertIn('usable', checklist_demo_content)
        self.assertIn('source-grounded', checklist_demo_content)
        self.assertIn('second task', checklist_demo_content)
        self.assertIn('https://www.usable.dev/features.html', checklist_demo_content)
        source_backed_handover_content = SOURCE_BACKED_HANDOVER_NOTE.read_text().lower()
        self.assertIn('a source-backed handover beats a polished answer', source_backed_handover_content)
        self.assertIn('usable', source_backed_handover_content)
        self.assertIn('source-backed', source_backed_handover_content)
        ...
        readback_feature_content = READBACK_FEATURE_NOTE.read_text().lower()
        self.assertIn('readback is part of the feature', readback_feature_content)
        self.assertIn('two-run test', readback_feature_content)
        self.assertIn('usable', readback_feature_content)
